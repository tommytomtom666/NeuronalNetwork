...
class neuronalNetwork:
    ...
    def __init__(self,inputnodes,hiddennodes,outputnodes,learningrate):
        #set number of nodes in each input, hidden, outpur layer
        self.inodes = inputnodes
        self.hnodes = hiddennodes
        self.onodes = outputnodes

        #link weight matrices, whi and who
        ...
        self.acitivation_function = lambda x : scipy.special.expit(x)
        pass
    ...
    def train(self, inputs_list, targets_list):
        ...
        hidden_outputs= self.acitivation_function(hidden_inputs)

        #calculate signals into final output layer
        final_inputs = numpy.dot(self.who, hidden_outputs)
        #calculate signals emerging from final output layer
        final_outputs = self.acitivation_function(final_inputs)
...
#create instance of neuronal network
...
for record in training_data_list:
    #split the rechord by the commas
...
qeuery = n.query((numpy.asfarray(all_values[1:]) / 255.0 *0.99)+0.01)
